...
class Admin::LabsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should assign lab to a new course" do
    put admin_course_lab_url(courses(:intro_to_jackassery),labs(:hello_world)),
      params: {
        lab: {
          course_id: courses(:advanced_jackassery).id
        }
    }
    
    assert_equal courses(:advanced_jackassery).id, assigns(:lab).course_id
    assert_redirected_to admin_course_lab_path(courses(:advanced_jackassery),assigns(:lab))
  end
