  ...
  module Upgrading
    module FakeRouter
      module ActionController
        module Routing
          class Routes
            def self.setup
             xxx@xxx.xxx = Rails::Upgrading::RouteRedrawer.new
            end
    ...
    class RouteRedrawer
      ...
      def initialize
       ...
       xxx@xxx.xxx = false
        ...
        self.class.stack = xxx@xxx.xxx
      end
      
      def root(options)
        debug "mapping root"
       xxx@xxx.xxx << FakeRoute.new("/", options)
      end
    ...
    class RouteObject
      def indent_lines(code_lines)
        if code_lines.length > 1
          code_lines.flatten.map {|l| xxx@xxx.xxx + "\n"
        else
          xxx@xxx.xxx
        end
      end
    ...
    class FakeNamespace < RouteObject
      ...
      def initialize(name, options = {})
       xxx@xxx.xxx = []
       xxx@xxx.xxx @options = name, options
       xxx@xxx.xxx = RouteRedrawer.indent
      end
      ...
      def to_route_code
        ...
        lines = ["namespace :#{@name}#{options} xxx@xxx.xxx {|r| r.to_route_code}, "end"]
    ...
    class FakeRoute < RouteObject
      ...
      def to_route_code
        xxx@xxx.xxx
          indent_lines ["match xxx@xxx.xxx
        else
          base = "match '%s' => '%s#%s'"
          extra_options = []
          ...
          xxx@xxx.xxx
            @options[:constraints] xxx@xxx.xxx
          end
          
          xxx@xxx.xxx
            @options[:via] xxx@xxx.xxx
          end

         xxx@xxx.xxx ||= {}
          base = (base % [@path, @options.delete(:controller), xxx@xxx.xxx || "index")])
          opts = xxx@xxx.xxx
    ...
    class FakeResourceRoute < RouteObject
      ...
      def to_route_code
        ...
        copied_options xxx@xxx.xxx { |k,v| ![:only, :except].member?(k) }
        unless copied_options.empty?
          copied_options_str = ", " + copied_options.map { |k, v| "#{k.inspect} => #{v.inspect}" }.join(",")
        end
        
        if !@children.empty? || @options.has_key?(:collection) xxx@xxx.xxx
          prefix = ["#{route_method} xxx@xxx.xxx do"]
          lines = prefix + custom_methods + xxx@xxx.xxx {|r| r.to_route_code}.join("\n"), "end"]
        
          indent_lines(lines)
        else
          base = "#{route_method} :%s#{copied_options_str}"
          indent_lines [base % xxx@xxx.xxx
        end
      end
    ...
    class RouteGenerator
      ...
      def generate
        @new_code xxx@xxx.xxx do |r|
          r.to_route_code
        end.join("\n")
        
        "#{app_name.underscore.classify}::Application.routes.draw xxx@xxx.xxx
      end
