  ...
  module Storage
    ...
    module Filesystem
      ...
      def to_file style = default_style
       xxx@xxx.xxx || (File.new(path(style)) if exists?(style))
      end
      alias_method :to_io, :to_file

      def flush_writes #:nodoc:
        logger.info("[paperclip] Writing files for #{name}")
       xxx@xxx.xxx do |style, file|
    ...
    module S3
      def self.extended base
        ...
        base.instance_eval do
          @s3_credentials = xxx@xxx.xxx
          @bucket         = @options[:bucket] xxx@xxx.xxx
          @s3_options     xxx@xxx.xxx || {}
          @s3_permissions xxx@xxx.xxx || 'public-read'
          @s3_protocol    = @options[:s3_protocol] || xxx@xxx.xxx == 'public-read' ? 'http' : 'https')
          @url            = ":s3_path_url" xxx@xxx.xxx
        end
      ...
      def s3_bucket
        @s3_bucket ||= s3.bucket(@bucket, xxx@xxx.xxx
      end
      ...
      # style, in the format most representative of the current storage.
      def to_file style = default_style
       xxx@xxx.xxx || s3_bucket.key(path(style))
      end
      alias_method :to_io, :to_file
      ...
      def flush_writes #:nodoc:
       ...
       xxx@xxx.xxx do |style, file|
          begin
            ...
            key.data = file
            xxx@xxx.xxx {'Content-type' => instance_read(:content_type)})
