...
END:VCALENDAR
EOS
   xxx@xxx.xxx = Icalendar.parse(src).first
    @event xxx@xxx.xxx
  end
  ...
  def test_dtstart_should_understand_icalendar_tzid
    xxx@xxx.xxx :icalendar_tzid)
  end
  
  def test_dtstart_tzid_should_be_correct
    puts "#{@event.dtstart.icalendar_tzid} xxx@xxx.xxx
    xxx@xxx.xxx
  end
...
EOS
  ...
  def test_dtstart_tzid_should_be_nil
    puts "#{@event.dtstart.icalendar_tzid.inspect} xxx@xxx.xxx
    xxx@xxx.xxx
  end
...
EOS
  ...
  def test_occurrences_after_with_start_before_start_at_should_return_count_occurrences
    assert_equal xxx@xxx.xxx 9, 2, 8, 30, 0, 0)).length
  end
                  
#  def test_occurrences_after_with_start_before_start_at_should_return_an_event_with_the_dtstart_as_the_first_event
#    xxx@xxx.xxx @event.occurrences_starting(Time.utc(1997, 9, 2, 8, 30, 0, 0)).first.dtstart.to_s
#  end
#  
#  def test_occurrences_after_with_start_before_start_at_should_return_events_with_the_correct_dtstart_values
#    expected = (0..9).map {|delta| xxx@xxx.xxx + delta).to_s}
#    assert_equal xxx@xxx.xxx 9, 2, 8, 30, 0, 0)).map {|occurence| occurence.dtstart.to_s}
#  end
end
