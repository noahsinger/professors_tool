  ...
  class RRule
    ...
    class Weekday
      def initialize(day, position)
       xxx@xxx.xxx @position = day, position
      end
    ...
    def initialize(name, params, value, parser)
     ...
     xxx@xxx.xxx = frequency_match[1]
     xxx@xxx.xxx = parse_date_val("UNTIL", value)
     xxx@xxx.xxx = parse_int_val("COUNT", value)
      raise Icalendar::InvalidPropertyValue.new("UNTIL and COUNT must not both be specified for RRULE values") if xxx@xxx.xxx @count].compact.length > 1
     xxx@xxx.xxx = parse_int_val("INTERVAL", value)
     xxx@xxx.xxx = {:bysecond => parse_int_list("BYSECOND", value)}
     xxx@xxx.xxx = parse_int_list("BYMINUTE",value)
     xxx@xxx.xxx = parse_int_list("BYHOUR", value)
     xxx@xxx.xxx = parse_weekday_list("BYDAY", value)
     xxx@xxx.xxx = parse_int_list("BYMONTHDAY", value)
     xxx@xxx.xxx = parse_int_list("BYYEARDAY", value)
     xxx@xxx.xxx = parse_int_list("BYWEEKNO", value)
     xxx@xxx.xxx = parse_int_list("BYMONTH", value)
     xxx@xxx.xxx = parse_int_list("BYSETPOS", value)
     xxx@xxx.xxx = parse_wkstart(value)
    end
    
    def to_ical
      result = xxx@xxx.xxx
      result << ";UNTIL=#{@until.to_ical}" xxx@xxx.xxx
      result << ";COUNT=#{@count}" xxx@xxx.xxx
      result << ";INTERVAL=#{@interval}" xxx@xxx.xxx
     xxx@xxx.xxx do |key, value|
        result << ";#{key.to_s.upcase}=#{value}" if value
      end
      result << ";WKST=#{@wkst}" xxx@xxx.xxx
      result.join
    end
  ...
  class Parser < Icalendar::Base
    ...
    def initialize(src)
      ...
      if src.respond_to?(:gets)
       ...
       xxx@xxx.xxx = StringIO.new(src.to_s, 'r')
      else
        raise ArgumentError, "CalendarParser.new cannot be called with a #{src.class} type!"
      end

      @prev_line xxx@xxx.xxx
      @prev_line.chomp! xxx@xxx.xxx

      @@logger.debug("New Calendar Parser: xxx@xxx.xxx
    end
    ...
    def next_line
      ...
      loop do
        nextLine xxx@xxx.xxx
       xxx@xxx.xxx "new_line: #{nextLine}"
     ...
     xxx@xxx.xxx "parsing..."
      ...
      while (line = next_line) 
        ...
        if fields[:name] == "BEGIN" and fields[:value] == "VCALENDAR"
          cal = parse_component
         xxx@xxx.xxx "Added parsed calendar..."
          calendars << cal
        end
      end
    ...
    def parse_component(component = Calendar.new)
     xxx@xxx.xxx "parsing new component..."
      ...
      while (line = next_line)
        ...
        if name == "END"
          ...
          orig_value = value
          xxx@xxx.xxx
            value xxx@xxx.xxx params, value)
          end
